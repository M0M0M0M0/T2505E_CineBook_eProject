...
import { Pencil, Building2, X as XIcon, RefreshCw } from "lucide-react";
...
export default function TheaterTable({
  ...
  const extractCities = (data) => {
    const uniqueCities = [
      ...new Set(data.map((t) => t.city || t.theater_city)),
    ].filter((city) => city);
  ...
  const filteredTheaters = theaters.filter(
    (theater) =>
      ...
      (selectedCity === "" ||
        (theater.city || theater.theater_city) === selectedCity)
  );
        ...
        >
          <div className="d-flex align-items-center mb-3 gap-2">
            ...
            <button
              ...
              title="Reload theaters"
            >
              <RefreshCw
                size={16}
                className={`me-2 ${isReloading ? "spinner-icon" : ""}`}
              />
