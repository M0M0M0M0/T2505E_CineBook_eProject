  ...
  <div className="bg-white rounded-xl shadow-lg overflow-hidden transition-transform duration-300 hover:scale-[1.02] mb-8">
    ...
    <div className="px-6 pb-2">
      ...
      {theater.movies.length > 0 ? (
        theater.movies.map(movie => <MovieCard key={movie.id} movie={movie} />)
      ) : (
...
export default function Theaters() {
  ...
  const filteredTheaters = selectedRegion === 'Tất cả'
    ? theatersData
    ...
    <div className="bg-gray-50 min-h-screen font-sans">
      <div className="container mx-auto px-4 py-8">
        <header className="text-center mb-10">
          <h1 className="text-4xl font-extrabold text-gray-800 tracking-tight">Lịch Chiếu Phim</h1>
          <p className="mt-2 text-lg text-gray-600">Chọn rạp và đặt vé ngay hôm nay!</p>
        </header>

        {/* --- Bộ lọc khu vực --- */}
        <div className="mb-8 flex justify-center">
          <div className="relative w-full max-w-xs">
            <label htmlFor="region-filter" className="sr-only">Chọn khu vực</label>
            <select
              id="region-filter"
              value={selectedRegion}
              onChange={handleRegionChange}
              className="appearance-none w-full bg-white border border-gray-300 text-gray-700 py-3 px-4 pr-8 rounded-lg leading-tight focus:outline-none focus:bg-white focus:border-sky-500 shadow-sm transition"
            >
              {regions.map(region => (
        ...
        <main>
          {filteredTheaters.length > 0 ? (
            ...
            <div className="text-center bg-white p-8 rounded-xl shadow-md">
              <h2 className="text-xl font-semibold text-gray-700">Không tìm thấy rạp chiếu phim</h2>
              <p className="text-gray-500 mt-2">Vui lòng thử chọn một khu vực khác.</p>
            </div>
          )}
        </main>
      </div>
